 ...
 * @author Thinh Pham
 */
public class CARSEntry extends NULLEntry {
    ...
    public CARSEntry(String[] args, int groupId) {
        super(groupId);
        ...
        handlingId = args[i++];
        gameName = args[i++];
        if (!GameConfig.ALIAS_III.equals(GameConfig.getAlias()))
            anims = args[i++];
        clazz = args[i++];
    ...
    public String toString() {
        return String.format("[%1$03d] [%2$s] %3$s", id, type, handlingId);
    }
}
